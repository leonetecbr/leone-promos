...
@section('content')
    ...
    <div class="dropdown my-4 col-md-6 mx-auto">
        ...
        <ul class="dropdown-menu w-100 text-center" aria-labelledby="dropdownMenuButton1">
            <li>
                <a class="dropdown-item{{ ($loja=='')?' active':''; }}" href="{{ route('cupons') }}">Todas</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Americanas')?' active':''; }}" href="{{ route('cupons') }}?loja=Americanas">Americanas</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Casas Bahia')?' active':''; }}" href="{{ route('cupons') }}?loja=Casas Bahia">Casas Bahia</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Consul')?' active':''; }}" href="{{ route('cupons') }}?loja=Consul">Consul</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Ponto')?' active':''; }}" href="{{ route('cupons') }}?loja=Ponto">Ponto</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Submarino')?' active':''; }}" href="{{ route('cupons') }}?loja=Submarino">Submarino</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Efácil')?' active':''; }}" href="{{ route('cupons') }}?loja=Efácil">Efácil</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Positivo')?' active':''; }}" href="{{ route('cupons') }}?loja=Positivo">Positivo</a>
            </li>
            <li>
                <a class="dropdown-item{{ ($loja=='Lenovo')?' active':''; }}" href="{{ route('cupons') }}?loja=Lenovo">Lenovo</a>
            </li>
        </ul>
    </div>
    <article id="cupons" class="container d-flex justify-content-around flex-wrap">
        ...
        <div class="cupom card col-lg-3-5 col-md-5 col-sm-10 col-12 mb-5" id="cupom-{{ $loop->index }}">
            ...
            <div class="card-body p-3 text-center mb-md-3">
                <div class="site mb-3">
                    <img src="{{ $cupom['store']['imagem'] }}" alt="{{ $cupom['store']['nome'] }}" class="loja-image">
                </div>
                ...
                <div class="code row col-11 mx-auto">
                    <input value="{{ $cupom['code'] }}" disabled="true" class="form-control text-center code-text" />
                </div>
            </div>
            <div class="final text-center p-3">
                <button data-link="{{ $cupom['link'] }}" class="btn btn-outline-danger w-75 mx-auto copy-redirect">Copiar e ir para a loja</button>
            </div>
        </div>
        @endforeach
    </article>
