...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ms-auto">
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='home')?'#':route('home') }}" class="nav-link{{ (Route::currentRouteName()=='home')?' active':''; }}">Início</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='cupons')?'#':route('cupons') }}" class="nav-link{{ (Route::currentRouteName()=='cupons')?' active':''; }}">Cupons</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='lojas')?'#':route('lojas') }}" class="nav-link{{ (Route::currentRouteName()=='lojas')?' active':''; }}">Lojas</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='categorias')?'#':route('categorias') }}" class="nav-link{{ (Route::currentRouteName()=='categorias')?' active':''; }}">Categorias</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='notificacoes')?'#':route('notificacoes') }}" class="nav-link{{ (Route::currentRouteName()=='notificacoes')?' active':''; }}">Notificações</a>
                    </li>
                    <li class="nav-item">
                        <a href="{{ (Route::currentRouteName()=='rastreio')?'#':route('rastreio') }}" class="nav-link{{ (Route::currentRouteName()=='rastreio')?' active':''; }}">Rastreio</a>
                    </li>
                </ul>
                ...
                <a href="{{ route('dashboard') }}" class="ms-3">
                    <button id="btn-admin" class="btn btn-outline-info mt-3 mt-md-0 float-start"><i class="fa-solid fa-lock"></i></button>
                </a>
                <a href="{{ route('logout') }}" class="ms-3">
                    <button id="btn-logout" class="btn btn-outline-danger mt-3 mt-md-0 float-end"><i class="fas fa-sign-out-alt"></i></button>
                </a>
                @endif
            </div>
        </nav>
    ...
    <footer id="rodape" class="text-center border-top p-3 mt-3 bg-light">
        ...
        <p class="fs-6">
            <span class="bolder">Políticas: </span> <a href="`{{ route('privacidade') }}" target="_blank" class="text-decoration-none">de Privacidade</a> | <a href="{{ route('cookies') }}" target="_blank" class="text-decoration-none">de Cookies</a>
        </p>
    </footer>
</body>
