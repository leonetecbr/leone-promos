...
class SentNotificationController extends Controller
{
    ...
    {
        $sent_notifications = SentNotification::orderBy('sent_at', 'DESC')->paginate(10, '*', 'page', $page);
        $notifications = $sent_notifications->items();

        return view('admin.sent_notifications', ['notifications' => $notifications, 'endPage' => $sent_notifications->lastPage(), 'page' => $page, 'groupName' => 1]);
    }
}
